 ...
 * Pure DOM, no framework. Styles live in styles.css (oc-* classes).
 */
...
function iconHtml(name?: string): string {
  ...
  return svg ? `<span class="oc-icon" style="width:18px;height:18px">${svg}</span>` : '';
}
...
export function showMenu(container: HTMLElement, x: number, y: number, items: MenuItem[], opts: { parent?: OpenMenu; anchor?: HTMLElement; minWidth?: number; className?: string } = {}): OpenMenu {
  ...
  const menu = el('div', { class: `oc-menu ${opts.className ?? ''}` });
  ...
  for (const it of items) {
    if (it.separator) { menu.appendChild(el('div', { class: 'oc-menu-sep' })); continue; }
    if (it.title) { menu.appendChild(el('div', { class: 'oc-menu-title', text: it.label ?? '' })); continue; }
    const row = el('div', { class: `oc-menu-item ${it.disabled ? 'oc-disabled' : ''}` });
    if (it.element) row.appendChild(it.element);
    else {
      row.innerHTML = `${it.checked !== undefined ? `<span class="oc-check">${it.checked ? '✓' : ''}</span>` : ''}${iconHtml(it.icon)}<span class="oc-menu-label"${it.color ? ` style="color:${it.color}"` : ''}>${escapeHtml(it.label ?? '')}</span>${it.shortcut ? `<span class="oc-shortcut">${escapeHtml(it.shortcut)}</span>` : ''}${it.submenu ? '<span class="oc-shortcut">›</span>' : ''}`;
    }
...
export function tooltip(target: HTMLElement, text: string | (() => string), container?: HTMLElement): void {
  target.addEventListener('mouseenter', () => {
    ...
    tipTimer = window.setTimeout(() => {
      ...
      if (!t) return;
      tipEl?.remove();
      tipEl = el('div', { class: 'oc-tooltip', text: t });
...
export class Dialog {
  ...
  constructor(readonly opts: DialogOptions) {
    this.el = el('div', { class: `oc-dialog ${opts.className ?? ''}` });
    if (opts.width) this.el.style.width = `${opts.width}px`;
    this.header = el('div', { class: 'oc-dialog-header' }, [el('span', { text: opts.title })]);
    const closeBtn = el('button', { class: 'oc-dialog-close', html: ICONS.close, title: 'Close' });
    closeBtn.style.width = '28px';
    closeBtn.addEventListener('click', () => this.close());
    this.header.appendChild(closeBtn);
    this.el.appendChild(this.header);
    if (opts.tabs?.length) {
      const tabs = el('div', { class: 'oc-tabs' });
      for (const t of opts.tabs) {
        const b = el('button', { class: 'oc-tab', text: t });
    ...
    this.body = el('div', { class: 'oc-dialog-body' });
    this.el.appendChild(this.body);
    if (opts.tabs?.length) {
      for (const t of opts.tabs) {
        const pane = el('div', { class: 'oc-tab-pane' });
    ...
    if (opts.buttons?.length) {
      this.footer = el('div', { class: 'oc-dialog-footer' });
      for (const b of opts.buttons) {
        const btn = el('button', { class: b.primary ? 'oc-primary' : 'oc-secondary', text: b.label });
        if (b.left) btn.classList.add('oc-left');
    ...
    if (opts.modal !== false) {
      this.backdrop = el('div', { class: 'oc-dialog-backdrop' });
  ...
  setTab(name: string): void {
    ...
    for (const [n, b] of this._tabButtons) b.classList.toggle('oc-active', n === name);
  }
...
export function showColorPicker(anchor: HTMLElement, current: string, onChange: (color: string) => void, opts: { opacity?: boolean; container?: HTMLElement } = {}): void {
  closeColorPopup();
  const container = opts.container ?? anchor.closest('.oc-root') as HTMLElement ?? document.body;
  const popup = el('div', { class: 'oc-color-popup' });
  ...
  const grid = el('div', { class: 'oc-swatches' });
  for (const sw of COLOR_SWATCHES) {
    const c = el('div', { class: 'oc-swatch', title: sw });
    c.style.background = sw;
    if (toHex({ ...rgb, a: 1 }).toUpperCase() === sw.toUpperCase()) c.classList.add('oc-active');
    c.addEventListener('click', () => {
      const p = parseColor(sw);
      rgb = { r: p.r, g: p.g, b: p.b };
      grid.querySelectorAll('.oc-swatch').forEach((x) => x.classList.remove('oc-active'));
      c.classList.add('oc-active');
      emit();
    });
    grid.appendChild(c);
  }
  popup.appendChild(grid);
  const row = el('div', { class: 'oc-opacity' });
  ...
  const hex = el('input', { class: 'oc-input', value: toHex({ ...rgb, a: 1 }) });
  ...
  if (opts.opacity !== false) {
    const orow = el('div', { class: 'oc-opacity' });
...
export function colorButton(value: string, onChange: (c: string) => void, opts: { opacity?: boolean } = {}): HTMLButtonElement {
  const btn = el('button', { class: 'oc-color-btn', title: value });
  const inner = el('span');
...
export function numberInput(value: number, onChange: (v: number) => void, opts: { min?: number; max?: number; step?: number; int?: boolean } = {}): HTMLInputElement {
  const inp = el('input', { class: 'oc-input', type: 'number', value: String(value) });
...
export function textInput(value: string, onChange: (v: string) => void, opts: { placeholder?: string; multiline?: boolean } = {}): HTMLInputElement | HTMLTextAreaElement {
  if (opts.multiline) {
    const ta = el('textarea', { class: 'oc-textarea', placeholder: opts.placeholder ?? '' });
    ta.value = value;
    ta.addEventListener('input', () => onChange(ta.value));
    ta.addEventListener('keydown', (e) => e.stopPropagation());
    return ta;
  }
  const inp = el('input', { class: 'oc-input', type: 'text', value, placeholder: opts.placeholder ?? '' });
...
export function selectInput(value: string | number, options: Array<{ value: string | number; label: string }> | string[], onChange: (v: string) => void): HTMLSelectElement {
  const sel = el('select', { class: 'oc-select' });
...
export function checkbox(value: boolean, onChange: (v: boolean) => void, label?: string): HTMLElement {
  const inp = el('input', { class: 'oc-checkbox', type: 'checkbox' });
  inp.checked = value;
...
export function lineWidthPicker(value: number, onChange: (v: number) => void): HTMLElement {
  ...
  for (const w of [1, 2, 3, 4]) {
    const b = el('button', { class: `oc-linewidth-opt ${w === value ? 'oc-active' : ''}`, title: `${w}px` });
    b.innerHTML = `<svg width="22" height="12" viewBox="0 0 22 12"><path d="M1 6h20" stroke="currentColor" stroke-width="${w}"/></svg>`;
    b.addEventListener('click', () => { btns.forEach((x) => x.classList.remove('oc-active')); b.classList.add('oc-active'); onChange(w); });
    btns.push(b);
    wrap.appendChild(b);
  }
  return wrap;
...
export function lineStylePicker(value: number, onChange: (v: number) => void): HTMLElement {
  ...
  for (const [v, title, dash] of styles) {
    const b = el('button', { class: `oc-linestyle-opt ${v === value ? 'oc-active' : ''}`, title });
    ...
    b.addEventListener('click', () => { btns.forEach((x) => x.classList.remove('oc-active')); b.classList.add('oc-active'); onChange(v); });
    btns.push(b);
    wrap.appendChild(b);
  }
  return wrap;
...
export function formSection(title: string): HTMLDivElement {
  return el('div', { class: 'oc-form-section', text: title });
}
...
export function renderPropertyForm(container: HTMLElement, defs: PropertyDef[], style: Record<string, any>, onChange: (key: string, value: any) => void): void {
  for (const def of defs) {
    ...
    if (def.dependsOn) {
      ...
      update();
      container.addEventListener('oc-change', update);
    }
...
export function button(label: string, opts: { icon?: string; title?: string; className?: string; onClick?: (e: MouseEvent) => void; iconSize?: number } = {}): HTMLButtonElement {
  const b = el('button', { class: `oc-btn ${opts.className ?? ''}`, title: opts.title ?? '' });
  if (opts.icon) {
    const sp = el('span', { class: 'oc-icon' });
